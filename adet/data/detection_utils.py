...
def transform_instance_annotations(
    ...
    annotation = d2_transform_inst_anno(  # annotation.get('boundary').shape  50 2 
        annotation,
        transforms,
        image_size,
    ...
    if "beziers_pts" in annotation:
        beziers_pts = transform_ctrl_pnts_annotations(annotation["beziers_pts"], transforms)
        annotation["beziers_pts"] = beziers_pts
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if "beziers_pts" in annos[0]:
        beziers_pts = [obj.get("beziers_pts", []) for obj in annos]
        instance.beziers_pts = torch.as_tensor(beziers_pts, dtype=torch.float32)
...
def build_augmentation(cfg, is_train): 
    ...
    augmentation = []
    augmentation.append(T.ResizeShortestEdge(min_size, max_size, sample_style))  # 限制长宽比

    if is_train:
        augmentation.append(T.RandomContrast(0.3, 1.7))  # T.RandomContrast：随机调整图像对比度。
        augmentation.append(T.RandomBrightness(0.3, 1.7))  # 亮度
        augmentation.append(T.RandomLighting(random.random() + 0.5))  # 光照
        augmentation.append(T.RandomSaturation(0.3, 1.7))  # 饱和度
