...
from adet.modeling.model.dn_components import compute_dn_loss
...
class SetCriterion(nn.Module): 
    ...
    def loss_labels(self, outputs, targets, indices, num_inst, log=False):
        ...
        shape[-1] += 1
        target_classes_onehot = torch.zeros(shape,  # bs 100 25 2
                                            dtype=src_logits.dtype, layout=src_logits.layout, device=src_logits.device)
        target_classes_onehot.scatter_(-1, target_classes.unsqueeze(-1), 1)
        target_classes_onehot = target_classes_onehot[..., :-1]  # bs 100 25 1
    ...
    def loss_beziers(self, outputs, targets, indices, num_inst):
        ...
        idx = self._get_src_permutation_idx(indices)  
        src_beziers = outputs['pred_beziers'][idx]  # n, 8  
    ...
    def get_loss(self, loss, outputs, targets, indices, num_inst, **kwargs): 
        loss_map = {
            ...
            'cardinality': self.loss_cardinality, # prints, but does not participate in the actual calculation
    ...
    def forward(self, outputs, targets, mask_dict=None, dn_args=None):
