...
class TransformerPureDetector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # ------------------------------------------------
        if cfg.MODEL.USEDN == True:
            self.scalar = cfg.MODEL.TRANSFORMER.SCALAR
            self.texts_noise_scale = cfg.MODEL.TRANSFORMER.TEXTS_NOISE_SCALE
            self.ctrl_points_noise_scale = cfg.MODEL.TRANSFORMER.CTRL_POINTS_NOISE_SCALE
            self.num_patterns = cfg.MODEL.TRANSFORMER.NUM_PATTERNS
            self.contrastive = cfg.MODEL.TRANSFORMER.CONTRASTIVE
        # ----------------------------------------------------
        ...
        if cfg.MODEL.USEDN == True:
            tgt_weight_dict = {
                "tgt_loss_ce": loss_cfg.POINT_CLASS_WEIGHT,
                "tgt_loss_texts_pos": loss_cfg.POINT_TEXT_WEIGHT,
                "tgt_loss_texts_neg": loss_cfg.POINT_TEXT_WEIGHT,
                "tgt_loss_ctrl_points": loss_cfg.POINT_COORD_WEIGHT,
                "tgt_loss_bd_points": loss_cfg.BOUNDARY_WEIGHT,
            }
            ...
            if loss_cfg.AUX_LOSS:
                ...
                # decoder aux loss
                for i in range(cfg.MODEL.TRANSFORMER.DEC_LAYERS - 1):
                    tgt_aux_weight_dict.update(
                        {k + f'_{i}': v for k, v in tgt_weight_dict.items()})
                tgt_weight_dict.update(tgt_aux_weight_dict)

            weight_dict.update(tgt_weight_dict)
    ...
    def forward(self, batched_inputs): 
        ...
        if self.training:
            ...
            # -------------------------------------------------------------
            if self.usedn:
                dn_args=(targets, self.scalar, self.texts_noise_scale, self.ctrl_points_noise_scale, self.num_patterns, self.contrastive)
            else:
                dn_args=None
            # -------------------------------------------------------------

            output, mask_dict = self.detection_transformer(images, dn_args)

            loss_dict = self.criterion(output, targets, mask_dict, dn_args) 
            ...
            output, mask_dict = self.detection_transformer(images)
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets: 
            ...
            # -----------------------------------------
            raw_beziers_pts = targets_per_image.beziers_pts  
            # -----------------------------------------
            ...
            # -------------------------------------------------------
            gt_beziers_pts = raw_beziers_pts.reshape(-1, 8, 2) / \
                         torch.as_tensor([w, h], dtype=torch.float, device=self.device)[None, None, :]
            # -------------------------------------------------------
            ...
            new_targets.append(
                {
                    "labels": gt_classes,
                    "beziers": gt_beziers,
                    "beziers_pts": gt_beziers_pts,  # for denoising training
